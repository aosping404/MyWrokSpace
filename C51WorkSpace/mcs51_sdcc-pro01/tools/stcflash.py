...
def find_serial_ports():
    ...
    if sys.platform == "win32":
        ...
        ports = [p for p in ports if "usbserial" in p or "USB" in p]
    else:
...
def map_com_port(port_name):
    """将 COM 端口名称映射到实际的设备路径"""
    if port_name.upper().startswith("COM"):
        # 尝试将 COM3 映射到 /dev/ttyUSB0, COM4 映射到 /dev/ttyUSB1 等
        try:
            com_num = int(port_name[3:])
            if sys.platform != "win32":
                # Linux/macOS: COM3 -> /dev/ttyUSB0 (COM3 对应索引 2, 即 ttyUSB0)
                usb_index = com_num - 1
                candidates = [
                ...
                for candidate in candidates:
                    if os.path.exists(candidate):
                        logging.info("映射 %s -> %s" % (port_name, candidate))
                        return candidate
        except ValueError:
            pass
    return port_name
...
def main(): 
    ...
    # 映射 COM 端口名称到实际设备路径
    actual_port = map_com_port(opts.port)
    
    # 如果端口不存在，尝试自动检测
    if not os.path.exists(actual_port) and sys.platform != "win32":
        available_ports = find_serial_ports()
        if available_ports:
            actual_port = available_ports[0]
            logging.info("端口 %s 不存在，自动使用 %s" % (opts.port, actual_port))
            if len(available_ports) > 1:
                logging.info("可用端口: %s" % ", ".join(available_ports))
        else:
            print("错误: 无法找到串口设备 %s" % opts.port)
            print("提示: 在 Linux 上，请使用 /dev/ttyUSB0 或 /dev/ttyACM0")
            print("提示: 在 Windows 上，请使用 COM1, COM2, COM3 等")
            sys.exit(1)
    
    print("通信端口：%s  最低波特率：%d bps" % (actual_port, opts.lowbaud))
    
    global highbaud_pre
    highbaud_pre = opts.highbaud
    try:
        with serial.Serial(port=actual_port,
        ...
        print("错误: 无法打开串口 %s" % actual_port)
        print("详细信息: %s" % str(e))
        if sys.platform != "win32":
            available_ports = find_serial_ports()
            if available_ports:
                print("可用端口: %s" % ", ".join(available_ports))
            else:
                print("未找到任何可用的串口设备")
        sys.exit(1)

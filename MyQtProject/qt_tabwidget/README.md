...
1. **更新翻译文件**：
   ...
   lupdate src/*.cpp include/*.h ui/*.ui -ts translations/qt_tabwidget_zh_CN.ts
   ```

2. **编辑翻译**：
   - 使用 Qt Linguist 打开 `translations/qt_tabwidget_zh_CN.ts`
   - 或使用文本编辑器直接编辑

3. **编译翻译**：
   ```bash
   # 编译翻译文件为 .qm 格式
   lrelease translations/qt_tabwidget_zh_CN.ts
   ```

4. **添加新语言**：
   - 复制 `translations/qt_tabwidget_zh_CN.ts` 为新语言文件

...
echo "=========================================="
echo "  Ubuntu-QT-tools Git 仓库初始化"
echo "=========================================="
echo ""
...
if git remote | grep -q "^origin$"; then
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        git remote set-url origin https://github.com/aosping404/Ubuntu-QT-tools.git
        echo "✓ 远程仓库地址已更新"
    fi
else
    echo "添加远程仓库..."
...
if [ -z "$(git log --oneline 2>/dev/null)" ]; then
    echo "创建首次提交..."
    git commit -m "first commit: Add Qt development tools"
    echo "✓ 首次提交完成"
else
